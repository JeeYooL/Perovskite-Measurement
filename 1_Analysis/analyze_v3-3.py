...
def delete_selected_samples():
    ...
    if not selected_items:
        messagebox.showwarning("Selection Needed", "Please select sample folder(s) to delete.")
        return
    ...
    if not folders_to_delete:
        messagebox.showwarning("Selection Needed", "Please select at least one sample folder.")
        return

    confirm = messagebox.askyesno("Confirm Delete", 
                                  f"Are you sure you want to delete the following {len(folders_to_delete)} sample(s)?\n\n" + 
                                  "\n".join(folders_to_delete[:10]) + 
                                  ("\n..." if len(folders_to_delete) > 10 else "") +
                                  "\n\nThis will remove them from all analysis and plots.")
    if not confirm:
        return
    ...
    for folder in folders_to_delete:
        ...
        for it_id in items_to_remove_dist:
            ...
            # dist_list_tree에서도 제거해야 함 (on_file_tree_select와 유사한 로직이 필요할 수 있으나 refresh_all_views에서 처리됨)
    ...
    messagebox.showinfo("Deleted", f"{len(folders_to_delete)} sample(s) have been removed.")
...
def refresh_all_views(df_to_display):
    ...
    for index, row in current_display_df.iterrows():
        ...
        display_name = (f"★ {file_name}" if is_best else file_name) + f" [{scan_dir}]"
        ...
        # [수정] 태그 적용
        tags = ('best_pce',) if is_best else ()
        file_tree.insert(folder_nodes[sample_folder], 'end', text=display_name, values=(full_path,), tags=tags)
...
def load_and_process_folder(folder_path=None):
    ...
    if not file_list: messagebox.showinfo("Info", "No .txt files found."); return
    all_data = []
    ...
    for i, file_path in enumerate(sorted(file_list)):
        try:
            ...
            data_lines, header_line, parameters = [], "", {}
            ...
            if file_content is None:
                print(f"Could not read file '{file_name}' with any tried encoding.")
                continue
            ...
            for line in line_iter: # Continue from where 'end' was found
                if ':' in line:
                    parts = line.split(':', 1)
            ...
            jv_data_cache[file_path] = jv_data # Cache the dataframe

            scan_direction = detect_scan_direction(file_path, jv_data)
            Rs, Rsh = calculate_resistances(jv_data)

            voc_v = parse_value(parameters.get('Voc (V)')); jsc_a_cm2 = parse_value(parameters.get('Jsc(A/cm2)'))
            ff_percent = parse_value(parameters.get('Fill factor (%)')); pce_percent = parse_value(parameters.get('Efficiency (%)'))
            jsc_ma_cm2 = None
            if isinstance(jsc_a_cm2, (int, float)): jsc_ma_cm2 = jsc_a_cm2 * 1000.0
            all_data.append({'Sample': sample_folder_name, 'File': file_name, 'FullPath': file_path,
                             'Voc (V)': voc_v, 'Jsc (mA/cm2)': jsc_ma_cm2, 'FF (%)': ff_percent, 'PCE (%)': pce_percent,
                             'Rs (Ω·cm²)': Rs, 'Rsh (Ω·cm²)': Rsh, 'Scan Direction': scan_direction})
        except Exception as e:
            print(f"Error processing '{file_path}': {e}") # Print full path for better debugging
        finally:
            # Update progress bar
...
def filter_last_four_files():
    ...
    try:
        # 1. 확실한 정렬을 위해 Sample과 File 기준으로 오름차순 정렬
        # (보통 측정 파일은 순차적으로 번호가 매겨지므로, 뒤쪽 4개가 최신 데이터입니다)
        current_display_df.sort_values(by=['Sample', 'File'], inplace=True)

        # 2. 각 Sample 그룹별로 뒤에서부터 4개(tail(4))만 추출
        filtered_df = current_display_df.groupby('Sample').tail(4)
        
        # 3. 화면 갱신
        refresh_all_views(filtered_df)
        
        # 결과 메시지
        messagebox.showinfo("Last 4 JV Data Applied", 
                            f"각 폴더별 마지막 4개 데이터만 남겼습니다.\n\n"
                            f"변경 전: {len(current_display_df)} 개\n"
                            f"변경 후: {len(filtered_df)} 개")

    except Exception as e:
        messagebox.showerror("Error", f"Failed to filter last 4 files:\n{e}")
# --- [추가 끝] ---
